...
equivClust2pdf <- function(equivList, ...) {
  UseMethod("equivClust2pdf")
}

auxPdf <- function(equivList, jobName, ylab) {
  lapply(equivList, function(clust) {
    plot(clust, hang = -1, main = jobName, sub = attr(clust, "sub"), ylab = ylab)
  })
}

equivClust2pdf.equivClust <- function(equivList, jobName, ylab, ...) {
  on.exit({
    dev.off(dev.list()["pdf"])
  })
  if (missing(jobName))
    jobName <- attr(equivList, "jobName")
  if (missing(ylab))
    ylab <- attr(equivList, "ylab")
  
  pdf(file = paste0(jobName, ".pdf", sep = ""), ...)
  plot(equivList, hang = -1, main = jobName, sub = attr(equivList, "sub"), ylab = ylab)
  return()
}

equivClust2pdf.iterEquivClust <- function(equivList, jobName, ylab, ...) {
  on.exit({
    dev.off(dev.list()["pdf"])
  })
  if (missing(jobName))
  ...
  pdf(file = paste0(jobName, ".pdf", sep = ""), ...)
  lapply(equivList, auxPdf, jobName = jobName, ylab = ylab)
}